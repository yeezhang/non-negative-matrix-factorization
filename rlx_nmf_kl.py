...
class RlxKLdivNMF(BaseEstimator, TransformerMixin):
    ...
    def __init__(self, n_components=None, tol=1e-6, max_iter=200, eps=1.e-8,
                 subit=10, random_state=None, init=None, rho=1, beta=0.0):
    ...
    def _initialize_nmf(self, X, n_components, init=None, eps=1e-6,
        ...
        if init == 'svd':
            return np.dot(U, np.diag(np.sqrt(S))), np.dot(np.diag(np.sqrt(S)), V)
    ...
    def fit_transform(self, X, y=None, weights=1., _fit=True,
        ...
        for n_iter in xrange(1, self.max_iter + 1):
            # Stopping condition
            error = self.error(X, Xplus, weights=weights)
            ...
            if return_errors:
                errors.append(error)
            W, alpha, Xplus = self._update(X, Xplus, W, alpha, _fit=_fit)
    ...
    def _update(self, X, Xplus, W, alpha, _fit=True, scale_W=False, eps=1.e-8):
        ...
        # update W
        #W = self._updated_W(X, W, self.components_, Xplus)
        H = np.matrix(self.components_)
        Q = np.dot(H, H.T) + self.beta*np.eye(self.n_components, self.n_components)
        W = np.dot(inv(Q), np.dot(H, Xplus.T + alpha.T/self.rho)).T
        if _fit:
            # update H
            Q = np.dot(W.T, W) + self.beta*np.eye(self.n_components, self.n_components)
            self.components_ = np.array(np.dot(inv(Q), np.dot(W.T, Xplus + alpha/self.rho)))
            #self.components_ = self._updated_H(X, W, self.components_, Q=Q)

        # update Xplus
        if sp.issparse(Xplus):
            WH = _special_sparse_dot(W, H, X)
        else:
            WH = np.dot(W,H)
        b = self.rho*WH - alpha -1
        Xplus = (b + np.sqrt(np.square(b) + 4*self.rho*X))/(2*self.rho)
        #print(Xplus)
        # update alpha
        alpha = alpha + self.rho* (Xplus - WH) 

        return W, alpha, Xplus
    ...
    def error(self, X, Xplus, weights=1., eps=1.e-8):
        X = check_array(X, accept_sparse='csr')
        Xplus = check_array(Xplus, accept_sparse='csr')
        if sp.issparse(X) and sp.issparse(Xplus):
            # Avoid computing all values of WH to get their sum
            return (np.multiply(
                X.data,
                np.log(np.divide(X.data + eps, Xplus.data + eps))
                )).sum() - X.data.sum() + Xplus.data.sum()
        elif sp.issparse(X):
            return generalized_KL(X.todense(), Xplus)
        else:
            return generalized_KL(X, Xplus)
